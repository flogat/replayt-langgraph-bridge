...
| `BridgeInvokeContextError` | Raised when `CompiledStateGraph.invoke` / `ainvoke` omits `context`, omits `runner`, passes `runner=None`, passes a non-`Runner`, or passes a `Runner` for a different `Workflow` than the compiled graph. `str(exc)` contains **`replayt bridge invoke context`**; `code` is **`missing_runner`** or **`runner_workflow_mismatch`**. See **[GRAPH_CONSTRUCTION_ERRORS.md](GRAPH_CONSTRUCTION_ERRORS.md)** §3.4. |
...
**Documented and CI-tested path:** Use synchronous **`CompiledStateGraph.invoke`** with **`initial_bridge_state`**, optional LangGraph **`config`** (e.g. **`thread_id`** when a **`checkpointer`** is set), and **`context={"runner": runner}`**. README examples, checkpoint guidance in **[CHECKPOINT_PERSISTENCE.md](CHECKPOINT_PERSISTENCE.md)**, and **`tests/`** all follow this pattern. If **`context`** omits **`runner`** or supplies **`None`**, the bridge raises **`BridgeInvokeContextError`** with substring **`replayt bridge invoke context`**; see **[GRAPH_CONSTRUCTION_ERRORS.md](GRAPH_CONSTRUCTION_ERRORS.md)** §3.4.
