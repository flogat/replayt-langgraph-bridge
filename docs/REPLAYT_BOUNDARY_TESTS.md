...
3. **CI / default suite** — Tests live under `tests/` and are collected by the same **`uv run pytest`** invocation documented for contributors and run in **`.github/workflows/ci.yml`** job **`test`** (Python 3.11 and 3.12). That job also runs **ruff** and the **mypy** smoke described in **README** and the workflow. No separate CI job is **required** for boundary tests. That job installs **`[dev]`** only; it must **not** require the optional **`demo`** extra (**[DESIGN_PRINCIPLES.md — Core vs demo extras](DESIGN_PRINCIPLES.md#core-vs-demo-extras-llm-clients-and-supply-chain)**).
