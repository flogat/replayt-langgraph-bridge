...
2. **Automated tests in CI** — Unit and **contract-style replayt boundary** tests for behavior we document; CI fails on regressions with clear logs (see **[DESIGN_PRINCIPLES.md](docs/DESIGN_PRINCIPLES.md)**). Import **replayt**, exercise **supported** public APIs, and use **actionable assertion messages** as specified in **[REPLAYT_BOUNDARY_TESTS.md](REPLAYT_BOUNDARY_TESTS.md)** (including product backlog acceptance mapping). GitHub Actions workflow **[`.github/workflows/ci.yml`](../.github/workflows/ci.yml)** runs job **`test`** (Python **3.11** and **3.12** matrix) with **`uv run pytest`** and **no path/marker filter**, matching **README** and **CONTRIBUTING.md**. The **test** job installs **`[dev]`** only from **`uv.lock`** (no **`demo`** extra), so CI matches integrators who do not opt into demo-only LLM client dependencies (**[DESIGN_PRINCIPLES.md — Core vs demo extras](DESIGN_PRINCIPLES.md#core-vs-demo-extras-llm-clients-and-supply-chain)**).
