...
**Shipped demos (today):** This repository **does not** yet include a committed runnable LLM sample (no `examples/` tree or equivalent that calls providers). The **`demo`** extra in **`pyproject.toml`** exists for packaging alignment and future samples. **Builder acceptance criteria** for the planned first-party sample live in **[BACKLOG_FIRST_PARTY_LLM_SAMPLE.md](BACKLOG_FIRST_PARTY_LLM_SAMPLE.md)** (Mission Control item `74a40ce9-cd64-4ab3-bda8-50296e094201`). When maintainers add that sample, it must stay **opt-in**, document **env vars**, **cost**, and **logging/redaction** in the README, and must **not** change the default CI contract above.
