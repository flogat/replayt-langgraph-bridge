...
**Disk-backed tests (CI):** **`tests/test_disk_checkpoint_sqlite_roundtrip.py`** exercises **`SqliteSaver`** on a real file under pytest’s **`tmp_path`**, with **`JSONLStore`** for the replayt plane, **`context={"runner": runner}`**, and a stable **`thread_id`**. The proof is **re-compile** (two **`compile_replayt_workflow`** calls sharing one on-disk saver), not a subprocess. **Linux** is what GitHub Actions runs; the test is **single-threaded** with **one** writer. On **Windows** / **WSL**, SQLite file locking can differ—follow upstream persistence docs for production concurrency. If a later Mission Control item is phrased as fixing a **pre-existing** failure in this module, the Builder must first rerun the default **`[dev]`** full suite; a green run means the branch already satisfies the disk-checkpoint contract and should not be changed just for churn. Normative acceptance and lockfile rules: **[BACKLOG_DISK_CHECKPOINT_SQLITE_ROUNDTRIP.md](BACKLOG_DISK_CHECKPOINT_SQLITE_ROUNDTRIP.md)** and §7 below.
...
- [x] **Tests (disk / SQLite):** Per **[BACKLOG_DISK_CHECKPOINT_SQLITE_ROUNDTRIP.md](BACKLOG_DISK_CHECKPOINT_SQLITE_ROUNDTRIP.md)** — **`langgraph-checkpoint-sqlite`** / **`SqliteSaver`**, **re-compile boundary** (two **`compile_replayt_workflow`** calls, same DB file and **`thread_id`**), default CI / **`[dev]`** / **`uv.lock`** contract. **CI platform assumptions** are in §3 **Disk-backed tests (CI)** above.
