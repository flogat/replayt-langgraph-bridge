...
| **Tested matrix** | **Python** 3.11 and 3.12 in GitHub Actions; each job runs **`uv sync --frozen --extra dev`** then **pytest** from the same resolved graph recorded in **`uv.lock`** (no **`demo`** extra). | `.github/workflows/ci.yml`; **[DEPENDENCY_LOCK_STRATEGY.md](DEPENDENCY_LOCK_STRATEGY.md)** |
