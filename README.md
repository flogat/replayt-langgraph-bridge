...
- **What CI exercises**: **Python** 3.11 and 3.12 jobs install the package with **`[dev]`** and run **`pytest`** (no path or marker filter)—**without** any optional **`demo`** extra. The same command runs **unit tests and contract-style replayt boundary tests** together; normative scope and acceptance mapping are in **[docs/REPLAYT_BOUNDARY_TESTS.md](docs/REPLAYT_BOUNDARY_TESTS.md)**. That proves the integrator-relevant install path stays green when demo-only LLM client dependencies are not present. **Today**, **replayt** and **langgraph** resolve to the **latest versions allowed by those ranges** on each run (no committed lockfile yet). **Target state**—committed **`uv.lock`** or hashed constraints, CI installs from that artifact, and security→lock workflow—is specified in **[docs/DEPENDENCY_LOCK_STRATEGY.md](docs/DEPENDENCY_LOCK_STRATEGY.md)**.
