...
- **Periodic lock refresh:** **`.github/workflows/uv-lock-refresh.yml`** (weekly schedule and **`workflow_dispatch`**) regenerates **`uv.lock`**, runs **`pip-audit`** and **`test`**-job checks, and proposes a PR when the lock changes. Spec: **[docs/BACKLOG_UV_LOCK_REFRESH_WORKFLOW.md](docs/BACKLOG_UV_LOCK_REFRESH_WORKFLOW.md)**.
...
- `BridgeWorkflowCompileError`, `BridgeGraphMappingError`, `BridgeTransitionError`, `BridgeRoutingError`, `BridgeInvokeContextError`: Compile, graph-mapping, and invoke-context errors (**[docs/GRAPH_CONSTRUCTION_ERRORS.md](docs/GRAPH_CONSTRUCTION_ERRORS.md)**). `BridgeLargeGraphWarning` (`UserWarning` subclass) may be emitted **once per process** after a successful compile when the workflow has many steps (threshold in **`replayt_langgraph_bridge.graph`**; see the same doc §4.3).
